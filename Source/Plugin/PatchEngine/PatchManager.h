...
///
/// Patches are `.cspatch` files (XML inside) split into two sources:
...
private:
    ...
    static constexpr const char *patchFileExtension = ".cspatch";
    static constexpr const char *patchRootTagName   = "CSPatch";

    // `bypass` is host-driven and should not be overridden by a patch load.
    // All other v2.0 params — including `uiMode` — are part of a patch so that
    // loading a Vibe-authored patch restores Vibe mode (Option A semantics).
