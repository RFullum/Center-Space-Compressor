...
namespace
{
    static constexpr int popupWidth     = 700;
    static constexpr int popupHeight    = 400;
    static constexpr int popupPadding   = 16;
    static constexpr int itemsPerColumn = 10;
    static constexpr int columnWidth    = 180;
    ...
    public:
        ...
        void paint(juce::Graphics &g) override
        {
            ...
            // Left-edge tick column for the currently-loaded patch.
    ...
    public:
        ...
        , onSelect      (std::move(onSelectIn))
        {
            ...
            {
                // Factory patches in orange (primaryAccent), user patches in
                // teal (secondaryAccent). Matches the Center Space palette.
            ...
            const int numCols = juce::jmax(1, (int) std::ceil((double) items.size() / itemsPerColumn));
    ...
    public:
        ...
        void paint(juce::Graphics &g) override
        {
            ...
            // Empty-state hint when no user patches exist yet.
            if (isEmpty)
            {
                g.setColour(resources.theme.textSecondary);
                g.setFont(juce::Font(juce::FontOptions("Helvetica", 14.0f, 0)));
                g.drawText("No patches yet. Use SAVE AS to create one",
...
PatchSelectionPopup::PatchSelectionPopup(GuiResources &res, PatchManager &pm)
: resources(res)
, patchManager(pm)
{}
                               ...
                               std::function<void(const juce::File &)> onFileChosen)
{
    ...
    // shared_ptr keeps the component alive across the lifetime of the modal.
    auto popup = std::make_shared<PatchSelectionPopupComponent>(resources
                                                                , patchManager.GetPatchList()
                                                                , patchManager.GetCurrentPatchFile());
    ...
    popup->enterModalState(true
                           , juce::ModalCallbackFunction::create(
                                 [popup, onFileChosen = std::move(onFileChosen)] (int /*result*/)
                                 {
                                     ...
                                     if (chosen.existsAsFile() && onFileChosen)
                                         onFileChosen(chosen);
                                 })
                           , false);
}
