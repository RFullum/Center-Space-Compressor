...
public:
    ...
    ~signal_base() override {
        if constexpr (std::is_same<Lockable,detail::null_mutex>::value)
            assert_not_emitting();

        disconnect_all();
    }

    signal_base(const signal_base &) : m_block(false) {}
    signal_base &operator=(const signal_base &) { return *this; }
...
private:
    ...
    // asserts if the signal is being emitted
    void assert_not_emitting()
    {
        // null mutex implementation does not handle well recursive connections
        // or being deleted while emitting
        // use a valid lockable type or remove recursion
        if constexpr (std::is_same<Lockable,detail::null_mutex>::value)
            assert(!m_emitting.load());
    }
...
private:
    ...
    std::atomic<bool> m_emitting{false};
};
